...
for i in $(seq 1 $(echo "$ids" | wc -l)); do
    ...
    echo "$id,\"$title\",\"$partyName\",\"$memberName\",$publicationDate,$lastModified" >> "/home/waybackMachine/log/$filename"
done

# Finds the names of the two newest files in the log directory
newest=$(find /home/waybackMachine/log -type f -printf "%T@ %p\n" | sort -nr | head -1 | awk '{print $2}')
secondNewest=$(find /home/waybackMachine/log -type f -printf "%T@ %p\n" | sort -nr | head -2 | tail -1 | awk '{print $2}')

# Compares the two files using 'cmp'
if ! cmp -s "$newest" "$secondNewest"; then
  #If there is a change, runs spnBills.sh
  /home/waybackMachine/spnBills.sh
fi
